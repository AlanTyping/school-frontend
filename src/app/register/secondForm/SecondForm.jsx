...
const SecondForm = ({ handleInputChange, handleDataSubmit }) => {
	...
	return (
		<form onSubmit={onSubmit} className="w-full flex flex-col items-center justify-center">
			...
			<select id='select-options' className="p-4 py-4 my-2 mb-8 bg-[#0f3b13] w-[90%] text-start rounded" {...register('role')} onChange={handleInputChange}>
			...
			<ReCAPTCHA onChange={handleCaptchaSubmission} ref={recaptchaRef} sitekey="6Lfg7OEoAAAAAO-0UlVU_kEtcTEp4b_U05YevLHi" />
			{showCaptchaMessage && <span>*Aprobar captcha</span>}

			<div className="w-full h-[auto] mt-8 flex items-center justify-center">
				<button className="bg-[#1b7423] py-3 w-[90%] 2xl:h-[5vh] rounded" type="submit">Submit</button>
			</div>
		</form>
	)
}
